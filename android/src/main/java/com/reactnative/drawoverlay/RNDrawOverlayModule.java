package com.reactnative.drawoverlay;
...
public class RNDrawOverlayModule extends ReactContextBaseJavaModule {
    ...
    private final int DRAW_OVER_OTHER_APP_PERMISSION_REQUEST_CODE = 1234;
    ...
    public void canDrawOverlays(Promise promise) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            promise.resolve(Settings.canDrawOverlays(this.reactContext));
        } else {
    ...
    @ReactMethod
    public void requestDisplayOverOtherAppsPermission(Promise promise) {
        mPromise = promise;
